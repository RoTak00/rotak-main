...
class Image
{
    ...
    public function __construct($registry)
    {
        ...
        if ($files) {
            foreach ($files as $file) {
                if (filemtime($file) < time() - self::CACHE_DUR) {
                    unlink($file);
                }
            }
        }
    ...
    {
        file_put_contents('log.log', $content, FILE_APPEND);
        $html = preg_replace_callback(
            '/<img\s+[^>]*src\s*=\s*"([^"]+)"[^>]*(?:width\s*=\s*"(\d+)")?[^>]*(?:height\s*=\s*"(\d+)")?[^>]*>/i',
            function ($matches) {
                file_put_contents('log.log', print_r($matches, true) . "\n", FILE_APPEND);
                $oldSrc = $matches[1];
